...
各ファイル、ソースコードを編集した際にインデントを統一します。
これまでのものに関しては今後編集する際に合わせていただければいいです。

現在 -> tab2個のものとtab4個のもの、その他のものが混在している。
今後 -> tab4個で統一

# タブの変更方法を簡単に紹介しときます。(Visual Studio Code.appの場合)

エンドバー内にタブのサイズ: 3(スペース: 4)のように表示されている部分をクリックします。

タブのサイズになっている方
-> タブになっている方は「タブによるインデント」をクリックし、サイズを「4」に変更してください。

スペースになっいる方
-> 「インデントをタブに変換」をクリック、その後「タブによるインデント」をクリックしサイズを「4」に変更してください
...
今回のプロジェクトの場合はこうである
git remote add origin https://github.com/Fujiya-San/Forest.git
